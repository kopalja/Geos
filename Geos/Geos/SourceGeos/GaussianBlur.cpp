...
void GaussianBlur::Process( __in double blur, __in const Image & rOrigin, __out Image * pBurImage )
{
	BYTE *red = new BYTE[ rOrigin.size / 3 ];
	BYTE *green = new BYTE[ rOrigin.size / 3 ];
	BYTE *blue = new BYTE[ rOrigin.size / 3 ];

	BYTE *red_t = new BYTE[ rOrigin.size / 3 ];
	BYTE *green_t = new BYTE[ rOrigin.size / 3 ];
	BYTE *blue_t = new BYTE[ rOrigin.size / 3 ];

	for (int i = 0, j = 0; i < rOrigin.size; i += 3, j++)
	{
		red[j] = rOrigin.buffer[i];
		green[j] = rOrigin.buffer[i + 1];
		blue[j] = rOrigin.buffer[i + 2];
	}

	gaussBlur_4( red, red_t, rOrigin.width, rOrigin.height, blur );
	gaussBlur_4( green, green_t, rOrigin.width, rOrigin.height, blur );
	gaussBlur_4( blue, blue_t, rOrigin.width, rOrigin.height, blur );
	...
	for (int i = 0, j = 0; i < rOrigin.size; i += 3, j++)
	{
		pBurImage->buffer[i] = red_t[j];
		pBurImage->buffer[i + 1] = green_t[j];
		pBurImage->buffer[i + 2] = blue_t[j];
	}
