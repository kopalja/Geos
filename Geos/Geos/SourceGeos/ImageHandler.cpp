...
ImageHandler::ImageHandler( const char* inputPath, const char* outputPath, HRESULT* hr )
{
...
HRESULT ImageHandler::DefineFormat( const char *inputPath )
{
	int i = 0;
	while (true)
	{
...
HRESULT ImageHandler::SetPaths( __in const char* inputPath, __in const char* outputPath, __out wchar_t *wStringIn, __out wchar_t *wStringOut )
{
	try
	{
						...
						if (SUCCEEDED(hr))
						{
							...
							if ( k == GUID_WICPixelFormat24bpp3Channels || k == GUID_WICPixelFormat24bppBGR || k == GUID_WICPixelFormat24bppRGB )
								byteDepth = 3;
								 ...
								 byteDepth = 4;
							else if ( k == GUID_WICPixelFormat8bppGray )
								byteDepth = 1;
							else
								hr = E_INVALIDARG;
...
{
	HRESULT hr = piFrameEncode->WritePixels( image->height, image->width * byteDepth, byteDepth * image->height * image->width, image->buffer );
