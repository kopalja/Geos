	...
	if ( SUCCEEDED( hr ) )
	{
		...
		if ( SUCCEEDED( hr ) )
		{
			...
			if ( SUCCEEDED( hr ) )
			{
				...
				GrayScale( const_cast<Image &>( *pOrigin ), pGrayImage );
				delete pOrigin;

				/* Aloc ResultImage from grayImage */
				Image *pResultImage = new Image( grayWidth >> 2, grayHeight >> 2, graySize >> 4, new BYTE[graySize >> 4] );
				GrayToResult( m_resultType, fourierReduction, skin, const_cast<Image &>( *pGrayImage ), pResultImage );
				delete pGrayImage;
...
void S3::GrayScale( __in const Image & pOrigin, __out Image *pGrayImage )
{
	UINT byteDepth = 0;
...
void S3::GrayToResult( __in ResultType resultType, __in int fourierReduction, __in bool skin, __in const Image & rGrayImage, __out Image *pResultImage )
{
	...
	else if ( resultType == ResultType::S3Image )
	{
