...
void Block::CountQuater( __in int quaterToCount, __in const Image & pImage )
{
	UINT *quater;
	int baseX = x;
	int baseY = y;
...
void Block::CountLine( __in int lineToCount, __in const Image & pImage )
{
	if ( lineToCount == 1 )
	{
...
inline int Block::CountSubBlock( __in int index, __in const Image & pImage)
{
