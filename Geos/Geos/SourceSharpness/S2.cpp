...
void S2::CreateS2( __in const Image & pGrayImage, __out Image* pS2Image )
{
...
void S2::CountFirstRow(Block* rowToCount, const Image & image)
{
...
void S2::CountRow( Block* rowToCount, Block* rowAbove, const Image & image, int y )
{
	rowToCount[0].x = 0;
	rowToCount[0].y = y << 2;
	for (int i = 0; i < 9; i++)
